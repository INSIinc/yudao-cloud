...
'显示顺序',
    ` data_scope ` TINYINT NOT NULL DEFAULT 1 COMMENT
...
(
    200
) DEFAULT '' COMMENT '路由地址',
    ` icon ` VARCHAR
(
    100
) DEFAULT '#' COMMENT '菜单图标',
    ` component ` VARCHAR
(
    255
) NULL COMMENT '组件路径',
...
(
    1
) NOT NULL DEFAULT 1 COMMENT '是否可见',
    ` keep_alive ` BIT
(
    1
) NOT NULL DEFAULT 1 COMMENT '是否缓存',
    ` always_show ` BIT
(
    1
) NOT NULL DEFAULT 1 COMMENT '是否总是显示',
    ` creator ` VARCHAR
(
    64
) DEFAULT '' COMMENT '创建者',
